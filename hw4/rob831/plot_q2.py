            ...
            for file in os.listdir(f"{root}/{dir}"):
                ...
                try:
                    ...
                    if 'Eval_AverageReturn' in ea.Tags()['scalars']:
                        ...
                        eval_all_returns[dir] = (steps, values)
                    if 'Train_AverageReturn' in ea.Tags()['scalars']:
                        ...
                        # 保存每个文件的 'Train_AverageReturn' 值
                        train_all_returns[dir] = (steps, values)
                except Exception as e:
                    print(f"Error processing {event_file}: {e}")

# 绘制所有文件的 'Eval_AverageReturn' 值
plt.figure(figsize=(10, 6))
for file_name, (steps, values) in eval_all_returns.items():
    plt.scatter(steps, values, label="Eval_AverageReturn",s=80)
for file_name, (steps, values) in train_all_returns.items():
    plt.scatter(steps, values, label="Train_AverageReturn",s=80)
