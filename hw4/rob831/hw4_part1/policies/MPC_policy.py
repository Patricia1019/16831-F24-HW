...
class MPCPolicy(BasePolicy):
    ...
    def sample_action_sequences(self, num_sequences, horizon, obs=None):
        if self.sample_strategy == 'random' \
            ...
            for i in range(self.cem_iterations):
                ...
                std = np.ones((horizon, self.ac_dim)) * (self.high - self.low) / 2
                for i in range(self.cem_iterations):
                    # Sample candidate sequences from a Gaussian
                    candidate_action_sequences = np.random.normal(mean, std, size=(num_sequences, horizon, self.ac_dim))
                    candidate_action_sequences = np.clip(candidate_action_sequences, self.low, self.high)

                    # Evaluate candidate action sequences and select elites
                    ...
                    mean = self.cem_alpha * elites.mean(axis=0) + (1 - self.cem_alpha) * mean
                    std = self.cem_alpha * elites.std(axis=0) + (1 - self.cem_alpha) * std
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        for model in self.dyn_models:
            ...
            all_sum_of_rewards.append(sum_of_rewards)

        return np.mean(all_sum_of_rewards, axis=0) # TODO
